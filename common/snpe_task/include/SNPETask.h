...
public:
    ...
    int process (std::vector<float>& input,
                 zdl::DlSystem::TensorMap& outputTensor);
    int process (float* pInput,
                 uint32_t inputSize,
                 float* pOutput,
                 uint32_t outputSize);
    int process(std::unique_ptr<zdl::DlSystem::ITensor>& inputTensor,
                zdl::DlSystem::TensorMap& outputTensor);
    int process(uint8_t** pInput, uint32_t* pInputSize, uint32_t inputNum,
...
private:
    ...
    std::unique_ptr<zdl::DlSystem::ITensor> createInputTensor(const zdl::DlSystem::TensorShape& shape,
    ...
    int copyOutputTensor(float* pOutput, uint32_t outputSize, zdl::DlSystem::TensorMap& outputTensor);
    int createUserBuffer(zdl::DlSystem::UserBufferMap& userBufferMap,
